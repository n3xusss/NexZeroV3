...
export default function Footer() {
  return (
    <footer className="border-t border-white/10 bg-[#0d0d0d]">
      <div className="mx-auto grid w-full max-w-360 grid-cols-1 items-start gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1fr_2fr_1fr] lg:items-center lg:px-10">
        <div className="flex items-center justify-center gap-2 lg:justify-start">
          <div className="relative h-9 w-9">
            <Image src="/nexus.png" alt="Nexus logo" fill sizes="36px" className="object-contain" />
          </div>
          <span className="font-sans text-base font-bold tracking-[0.12em] text-white sm:text-lg">
            NEX<span className="text-red-600">ZER</span>O
          </span>
        </div>

        <div className="text-center">
          <p className="font-ocr text-sm uppercase tracking-[0.16em] text-white sm:text-base lg:text-[1.1rem] lg:leading-8">
        ...
        <div className="flex flex-col items-center gap-5 text-center lg:items-end lg:text-right">
          <div>
            <p className="mb-1 text-[0.72rem] uppercase tracking-widest text-zinc-500">Contact us</p>
            <a
              href="mailto:nexus@estin.dz"
              className="border-b border-white/15 pb-px text-[0.82rem] text-zinc-300 transition hover:text-white"
            >
              nexus@estin.dz
            </a>
          </div>
          ...
          <div>
            <p className="mb-2 text-[0.72rem] uppercase tracking-widest text-zinc-500">Follow us</p>
      ...
      <div className="border-t border-white/10 px-4 py-3 text-center text-[0.72rem] tracking-[0.08em] text-zinc-600">
...
function SocialIcon({ href, label, children }) {
  return (
    <a
      href={href}
      aria-label={label}
      className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10 text-zinc-400 transition hover:bg-red-600 hover:text-white"
    >
      {children}
    </a>
  );
